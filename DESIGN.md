...
清晰、可信、可追溯的物流运营工作台。界面以蓝灰色中性表面承载六个关键货物字段，缺失使用语义红、冲突使用琥珀色；签名体验是“先补缺失、再裁决冲突、最后确认装柜规则”的短路径。
...
- 工作台使用单列问题分组，卡片内部桌面端三列、小于 `lg` 时堆叠。
...
- 结构：货物定位、问题与原值、系统处理与依据三区。
- 状态：缺失、冲突、已接受、已修改、已跳过。
...
### SolutionComparisonDialog

- 入口：规划完成页右上角只保留“方案对比”一个操作。
- 流程：上传人工方案、解析箱件号、对齐系统方案、展示对比评分。
- 结果：以柜号为行展示人工与系统箱件号范围和件数，评分口径同步说明。
- 状态：上传、解析、结果均在同一弹窗完成，不跳转新页面。

## 6. Motion & Interaction

- 微交互使用 `transition-colors` / `transition-all`，时长沿用 Tailwind 默认值。
- 解析动画只使用透明度、位移和脉冲，不修改装柜计算动画。
- 方案对比解析动画只在读取人工方案时运行，完成后切换为静态结果。
